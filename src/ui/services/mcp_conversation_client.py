...
class MCPConversationClient:
    ...
    async def query_agents(
        ...
        """Query specific agents in a session and get their responses"""
        try:
            session = await self._get_session()
            payload = {
                "session_id": session_id,
            ...
            async with session.post(
                f"{self.base_url}/agents/query", json=payload
            ) as response:
                if response.status == 200:
                    result = await response.json()
                    logger.info(
                        f"Agents queried via MCP: "
                        f"{len(result.get('responses', {}))} responses"
                    )
                    return result
                else:
                    error_text = await response.text()
                    logger.error(f"MCP agent query failed: {error_text}")
                    raise Exception(f"MCP server error: {error_text}")

        except Exception as e:
            logger.error(f"Failed to query agents via MCP: {e}")
            raise
    ...
    def query_agents_sync(
        ...
        """Synchronous wrapper for query_agents"""
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                return loop.run_until_complete(
                    self.query_agents(session_id, message, target_agents, context)
                )
            finally:
                loop.close()
        except Exception as e:
            logger.error(f"Failed to query agents synchronously: {e}")
            raise
